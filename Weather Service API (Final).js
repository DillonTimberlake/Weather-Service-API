...
// -------------------------------------------------------------------
// added cache results per (lat, lon) for, 5–15 minutes
// -------------------------------------------------------------------
const cache = new Map();
...
// -------------------------------------------------------------------
// added a retry mechanism for resiliancy against transient NWS errors
// -------------------------------------------------------------------
async function nwsGetWithRetry(url, retries = 1) {
  try {
    return await nwsGet(url);
  } catch (err) {
    if (retries > 0) return nwsGetWithRetry(url, retries - 1);
    throw err;
  }
}
