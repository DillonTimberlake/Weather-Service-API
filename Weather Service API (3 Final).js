...
// -------------------------------------------------------------------
// NWS API helpers 
// added my own user agent to comply with NWS API guidelines: https://www.weather.gov/documentation/services-web-api#user-agent
// -------------------------------------------------------------------
const NWS_BASE = "https://api.weather.gov";
const NWS_HEADERS = {
  "User-Agent": "WeatherForecastServer/1.0 (dillontimberlake@icloud.com)",
...
async function getForecast(lat, lon) {
  ...
  // Changed "Today" to accomodate NWS naming conventions, which may not always include a "Today" period.
  const now = new Date();

const todayPeriod =
  periods.find(p => p.isDaytime && new Date(p.startTime) >= now) ??
  ...
  // NWI API already returns farenheit when using the temperature property, 

  if (typeof temp !== "number") {
  throw new Error("Invalid temperature from NWS");
}
